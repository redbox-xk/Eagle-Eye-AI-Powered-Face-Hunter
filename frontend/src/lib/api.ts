...
async function req<T>(path: string, opts?: RequestInit, retries = 2): Promise<T> {
...
export const api = {
  ...
  agentStatus: (id: string) => req<any>(`/agents/${id}`),
  ...
  addNode: (entity: string, attributes?: any) =>
    req<any>('/knowledge/node', {
      method: 'POST',
      body: JSON.stringify({ entity, attributes }),
    }),
  ...
  connectionProbability: (from_id: string, to_id: string) =>
    req<any>(`/knowledge/probability?from_id=${from_id}&to_id=${to_id}`),
}
